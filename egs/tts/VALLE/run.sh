...
options=$(getopt -o c:n:s --long gpu:,config:,infer_expt_dir:,ar_model_ckpt_dir:,infer_output_dir:,infer_mode:,infer_test_list_file:,infer_text:,infer_text_prompt:,infer_audio_prompt:,model_train_stage:,name:,stage:,resume:,resume_from_ckpt_path:,resume_type:,infer_output_path:,infer_strategy_mode:,infer_SNR: -- "$@")
...
while true; do
  case $1 in
    ...
    # [Only for Inference] Setup the output file path if specified
    --infer_output_path) shift; infer_output_path=$1; shift ;;
    # [Only for Inference] Setup the strategy mode if specified
    --infer_strategy_mode) shift; infer_strategy_mode=$1; shift ;;
    # # [Only for Inference] Setup the SNR if specified
    --infer_SNR) shift; infer_SNR=$1; shift ;;
...
if [ $running_stage -eq 3 ]; then
    ...
    if [ "$infer_mode" = "single" ]; then
        ...
        infer_text=""
        infer_text_prompt=""
        infer_audio_prompt=None
    fi

    if [ -z "$infer_output_path" ]; then
    infer_output_path="${infer_output_dir}/default_output_filename.extension"
    fi

    if [ -z "$infer_strategy_mode" ]; then
    infer_strategy_mode="clean"
    fi
    ...
    CUDA_VISIBLE_DEVICES=$gpu accelerate launch "$work_dir"/bins/tts/inference.py \
        ...
        --strategy_mode $infer_strategy_mode \
        --SNR $infer_SNR 
