...
class LDPCQAMCombinedTransmission_random:
    ...
    def channel_layer(self, qam_symbols, phoneme_length, acoustic_length):
        ...
        if gain is not None:
            ...
            qam_symbols[:partition_indices[0]] *= gain_shuffled[:partition_indices[0]]
            for i in range(1, 9):
                qam_symbols[partition_indices[i-1]:partition_indices[i]] *= gain_shuffled[partition_indices[i-1]:partition_indices[i]]
        ...
        if self.channel_type in [2, 3]:
            qam_symbols_demodulated = qam_symbols_noisy / gain_shuffled
        else:
...
if __name__ == "__main__":
    ...
    # Sample input data
    input_phoneme_matrix = np.random.randint(0, 256, size=(3, 1))
    input_acoustic_matrix = np.random.randint(0, 1024, size=(10, 8))

    # Set SNR
    snr_db = 25

    # Initialize the transmitter
    transmitter = LDPCQAMCombinedTransmission_random(input_phoneme_matrix, input_acoustic_matrix, H, G, snr_db=snr_db, gain_mean=0, gain_std=1, channel_type=2)
    ...
    transmitted_output_acoustic = transmitted_output_acoustic.reshape(original_shape, order='F')

    transmitted_output_acoustic = transmitted_output_acoustic.flatten()
    ...
    print("Comparison report for acoustic data:", compare_and_report(input_acoustic_data, transmitted_output_acoustic))

    print('input_acoustic_data:', input_acoustic_data)
    print('transmitted_output_acoustic:', transmitted_output_acoustic)
